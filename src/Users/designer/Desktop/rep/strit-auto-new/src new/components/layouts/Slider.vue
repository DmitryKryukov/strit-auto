    ...
    this.slider = new KeenSlider(this.$refs.slider, {
      ...
      move: (s) => {
        if (this.cardFlipAnimation) {
          const progress = s.details().position
          const absoluteProgress = progress / window.innerWidth + 1
          this.$refs.slider.style.setProperty(
            '--rotate',
            (this.maxAngle * (1 - absoluteProgress)).toFixed(2) + 'deg'
          )
        }
      },
    })
      ...
      if (window.innerWidth < 450) {
        ...
        slides.forEach((s, i) => {
          s.firstChild.style.transform =
            'rotate(calc(var(--rotate) + ' + 10 * i + 'deg)'
        })
      } else {
        this.cardFlipAnimation = false
        slides.forEach((s) => {
          s.firstChild.style.transform = 'rotate(0deg)'
        })
      }
    },
  },
